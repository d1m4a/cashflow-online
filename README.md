# Cashflow Online MVP

Минимально жизнеспособный прототип онлайн-игры "Денежный поток" с комнатами, ходами, сделками и базовой финансовой моделью.

## Возможности MVP

- Создание комнаты и подключение по коду.
- До 4 игроков в комнате.
- Выбор стартовой профессии с разными доходами, расходами, наличными и долгами.
- Очередность ходов, бросок кубика и игровое поле.
- События: зарплата, возможность, расход, рынок, благотворительность, сокращение.
- Выбор малой или крупной сделки при попадании на клетку возможности.
- Малые и крупные сделки с покупкой за наличные или с кредитом.
- Рыночные предложения на продажу активов.
- Закрытие обязательств при наличии достаточных наличных.
- Переход на Fast Track, когда пассивный доход покрывает расходы.
- Мечта игрока, отдельные клетки Fast Track и победа через покупку мечты.
- Чат комнаты.
- Real-time обновление комнаты без ручного обновления страницы.
- Условие выхода из крысиных бегов: пассивный доход стал не меньше расходов.
- Условие победы: игрок на Fast Track покупает мечту или достигает целевого Fast Track дохода.
- Единый серверный источник правил для поля и профессий (`/api/rules`).
- Тесты для ключевых правил финансовой модели.

## Запуск

Требуется Node.js 18 или новее.

```bash
npm start
```

После запуска открой:

```text
http://localhost:3000
```

Для проверки мультиплеера создай комнату и открой второй браузер или вкладку в приватном режиме.

## Проверка

```bash
npm test
```

## Структура проекта

- `backend/server.js` - HTTP-сервер, API комнат и раздача frontend.
- `frontend/index.html` - разметка лобби и игрового экрана.
- `frontend/app.js` - клиентская логика и обновление интерфейса.
- `frontend/styles.css` - тёмная UI-тема по игровому макету.
- `shared/gameRules.js` - правила, состояние игры и действия игроков.
- `test/gameRules.test.js` - тесты ядра правил.
- `docs/MVP.md` - границы текущего MVP и план развития.

## Подготовка к публикации в git

```bash
git init
git add .
git commit -m "Initial cashflow online MVP"
git branch -M main
git remote add origin <repo-url>
git push -u origin main
```
